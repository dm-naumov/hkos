...
def main() -> int:
    ...
    items = [
        Knowledge(title="TCP redirect works via nftables", body="meta l4proto tcp redirect to :12345", tags=["tcp", "nftables"]),
        Knowledge(title="UDP traffic bypasses the proxy", body="cause: rule matches tcp only; fix: add tproxy rule", kind="negative"),
        Knowledge(title="Decision: use fwmark for routing", body="fwmark 0x1 lookup 100 is stable across reboots", category="DECISION"),
    ]
    ...
    snapshot = snapshots.create(project.id, reason="demo", comment="quickstart", author="demo", force=True)
