...
class TestReleaseAcceptance:
    ...
    def test_release_scenario(self, tmp_path: Path) -> None:
        ...
        executor = MigrationExecutor({"001_mig": lambda step: versions.__setitem__(0, 2)})
        backup = BackupManager(tmp_path, keep_n=3)
        rollback = RollbackManager(tmp_path)
        validator = MigrationValidator(ctx.repos, ctx.index, snapshots, lambda pid: versions)
        detector = SchemaDetector(registry, lambda pid: versions)
        manager = MigrationManager(detector, registry, executor, backup, rollback, validator, ctx.index, snapshots)
        api = MigrationEngine(manager, MigrationHistory(), ctx.repos, ctx.index, snapshots, validator, lock_path=tmp_path / "migration.lock")
        api.migrate(); api.rollback()
        ...
        assert any("FailureRC" in t for t in titles)
